...
- [ ] T001 Read current `samelhag.dev/Components/Pages/About.razor` in full to understand existing Razor structure, class names, and loop logic
- [ ] T002 [P] Read current `samelhag.dev/Components/Pages/About.razor.css` in full to capture all existing rule names and patterns before rewriting
- [ ] T003 [P] Read current `samelhag.dev/Components/Pages/About.razor.cs` to confirm `GetCategoryClass()` switch and `_milestones` list — verify no changes needed
...
- [ ] T012 [US2] In `samelhag.dev/Components/Pages/About.razor.css`, append the `@keyframes glow-pulse` animation (0%/100%: box-shadow 0 0 8px rgba(var(--cat-r),...,0.5); 50%: box-shadow 0 0 20px rgba(var(--cat-r),...,0.9))

- [ ] T013 [US2] In `samelhag.dev/Components/Pages/About.razor.css`, append `.timeline-item.is-current::before` rule overriding dot to 18px × 18px, left -2.5rem, and applying `animation: glow-pulse 2s ease-in-out infinite`

- [ ] T014 [US2] In `samelhag.dev/Components/Pages/About.razor.css`, append `.timeline-card.is-current` rule: border-left-color rgba(var(--cat-r),...,1.0) !important, box-shadow 0 4px 32px rgba(var(--cat-r),...,0.25) !important — making the current card's left accent brighter than resting cards
...
- [ ] T015 [US3] In `samelhag.dev/Components/Pages/About.razor.css`, append `@media (max-width: 600px)` block: `.timeline-track` padding-left 2.5rem; `.timeline-track::before` left 0.75rem; `.timeline-item::before` left -1.85rem; `.timeline-item` padding-bottom 1.5rem

- [ ] T016 [US3] In `samelhag.dev/Components/Pages/About.razor.css`, append `@media (prefers-reduced-motion: reduce)` block: `.timeline-item::before` animation none !important; `.timeline-card` transition none !important
...
- [ ] T017 [P] Verify `samelhag.dev/Components/Pages/About.razor` line count is ≤ 300 lines (Constitution Principle 5 gate)
- [ ] T018 [P] Verify `samelhag.dev/Components/Pages/About.razor.css` contains no leftover `conveyor-*` class references (grep/search to confirm clean rename)
- [ ] T019 Run `dotnet build samelhag.dev/samelhag.dev.csproj` — must produce **0 errors, 0 warnings**
- [ ] T020 Run the app (`dotnet run --project samelhag.dev.AppHost`), navigate to `/about`, and verify all items in `specs/003-about-timeline-cleanup/quickstart.md` visually
